...
#define r  0.6   //0.5length of object
#define a 0.008   //width of object
#define l  0.4   //  length of cable
#define M  (0.45)  //Mass of object
...
//    std::cout<< "a  " << I<<std::endl;
    ...
    for(int i=0;i<this->x_sigmavector_size;i++){
        ...
        double alpha_p = (r/I)*(FF_net*sin(theta_p+theta_c)-FL_net*sin(theta_p+theta_d));
...
//        std::cout << "alphap"<<alpha_p<<std::endl;
       ...
       // std::cout << "alpha_p" <<  alpha_p <<std::endl;
        ...
        // +alpha_p*r*cos(theta_p)
        // +alpha_p*r*sin(theta_p)
        predict_sigma_state(ap_x,i) = acx  +alpha_p*r*cos(theta_p)   + omega_p*omega_p*r*sin(theta_p);
        predict_sigma_state(ap_z,i) = acz  +alpha_p*r*sin(theta_p) - omega_p*omega_p*r*cos(theta_p);
